...
use crate::{process::{Error, ErrorFault, GeneratedInfo, Group, Processor, Result, assert_internal, connect::{CircleTree, IsolinesMap}, failed_assert_internal, isolines::{NodeOnEdge, OnEdge}, process_vec3::PVec3, sizes_calculator::FixedHookCalculator}, viewer::DisplayCommand};
...
pub enum StitchCommand {
    ...
    MagicCircle(usize),
    NextRow,
    // Repeat(Box<StitchCommand>, usize),
}

#[derive(PartialEq)]
enum InternalStitchCommand {
    MoveCurr,
    MoveNext,
    NextRow,
}
...
impl StitchCommand {
    fn from_internal(stitches: Vec<InternalStitchCommand>, magic_circle: usize) -> Vec<Self> {
        use InternalStitchCommand as ISC;
        let mut result = vec![Self::MagicCircle(magic_circle)];
        let mut stitches = VecDeque::from(stitches);
        let mut curr = Self::NextRow;
        while let Some(stitch) = stitches.pop_front() {
            let moving = match stitch {
                ISC::MoveCurr => Move::Curr,
                ISC::MoveNext => Move::Next,
                ISC::NextRow => {
                    result.push(curr);
                    curr = Self::NextRow;
                    continue;
            ...
            let overwrite_curr = match curr {
                Self::Single => {
                    if let Some(next) = stitches.front() && !matches!(next, ISC::NextRow) && *next != stitch {
                        stitches.pop_front();
                ...
                Self::Inc(_) => todo!(),
                Self::Dec(_) => todo!(),
                Self::RepInc(_, _) => todo!(),
                Self::RepDec(_, _) => todo!(),
                Self::RepSingle(_) => todo!(),
                Self::MagicCircle(_) => todo!(),
                Self::NextRow => todo!(),
            };
        }
        todo!()
...
impl Processor {
    ...
    fn get_rev_trav_intersection(&self, start: PVec3, start_face: usize, isolines_map: &IsolinesMap, current_isoline: usize) -> Result<(PVec3, f32, (usize, usize, usize))> {
        ...
        let f = self.get_face_point_rev_trav(start, start_face).1.unwrap();
        ...
        let (((i,j), (node, k), poss), geo1, geo2) = loop {
            ...
            let (_, Some(curr)) = nodes.get(&prev).unwrap() else {
                ...
                // std::thread::sleep(std::time::Duration::from_secs_f32(1.0));
                // This can happen if self.get_face_point_rev_trav returns a point past the previous isoline -
                // this happens if the mesh isnt dense enough, or due to a missed intersection.
                return Err(Error {
                    issue: "Mesh wasn't dense enough, or internal error.".to_string(),
                    fault: ErrorFault::File,
                    solution: "Chose a denser STL file, a different seed point, or increase relative stitch size (by decreasing diameter or increasing hook size).",
                });
            };
